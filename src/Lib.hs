...
types s = toTypeArray s <$> typePointers s
  where
    ...
    toTypeArray source offset =
      B.splitAt offset source 
          & \(_, d) -> B.takeWhile (/= 0x50) d 
          & B.unpack 
          & fmap toChar 
          & capitalized
...
spriteOffset :: (Bits a, Num a) => Int -> B.ByteString -> a
spriteOffset idx s = shiftL (getBank idx) 14 + ptr idx s .&. 0x3FFF
    where 
    getBank a 
        | a < 0x1F  = 0x9
        | a < 0x4A  = 0xA
    ...
    ptr n src = 
      pokemonBaseStats src & (!! n) & ((!! 0xB) &&& (!! 0xC)) & process
